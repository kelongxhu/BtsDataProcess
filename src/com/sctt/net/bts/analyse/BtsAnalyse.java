...
public class BtsAnalyse extends TimerTask {
	...
	public Cell ruleCell(Cell cell) {
		try {
			...
			// 计算是否高铁覆盖
			int cqFlag = 0;
			boolean isHighFlag = isHighBts(splitName[4]);
			boolean isTunel = false;
			String tunelStr = "";
			if (isHighFlag) {
				cqFlag = getCqFlag(splitName, 5);// 产权开始标记
	 ...
	 * @return
	 */
	public boolean isHighBts(String str) {
	...
	public int getCqFlag(String[] splitName, int startFlag) {
		// 系统号_BTS序号_小区序号_市/县/区+小区名_高铁信息标识_室分/隧道_拉远_基站产权标识_传输产权标识_维护等级_直放站标识_小区功分信息标识
		int cqFlag = 0;
	...
	public Bts ruleBbu(Bts bts, Map<String, BtsSite> noIndoorMap,
		...
		try {
			...
			// 是否高铁标识
			boolean highFlag = isHighBts(array[2]);
			if (highFlag) {
			...
			if ("BBU1".equals(array[1])) {
				if (btsName.contains("共站")) {
					...
					if (!isRight(circuitRoom_ownership))
						return null;
					if (!isRight(Trans_ownership))
						return null;
				}
			} else {
				// 如果BBU2以后一定得有共站
	...
	public void test() {
		Cell cell = new Cell();
		cell.setName("1_363_2_龙里中铁生态俱乐部_电_电_C");
		cell.setBtsName("龙里中铁生态俱乐部");
		Cell ruleCell = ruleCell(cell);
